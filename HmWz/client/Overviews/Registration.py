...
from discord import Embed, Interaction, Role, ButtonStyle, utils, RawMessageDeleteEvent, InteractionResponded, TextChannel, Message, HTTPException, Forbidden, NotFound
...
class RegistrationOverview(BasicOverview):
    ...
    def __init__(self, guild, services, client):
        ...
        self.registration: wz.RegistrationRecord = None
        self.configured_roles: wz.RolesRecords = None
        self.registrations: wz.RegistrationsRecords = None
        self.list_records: wz.ListRecords = None
        self.total_registrations : int = 0
        self.permanent_registrations : int = 0
        self.non_permanent_registrations : int = 0
        self.max_registrations_per_list : int = 40
    ...
    async def create_registrations_embed(self, title: str, description: str) -> Embed:
        try:
            ...
            self.logger.debug(f"{self.log_context} Generated registrations list embed")
            return embed
        except Exception as e:
            self.logger.exception(f"{self.log_context} Failed to generate registration embed: {e}")
            return Embed(title=title, description=description, color=self.client_color)
        
    async def create_registrations_list(self, registrations: wz.RegistrationsRecords, chunk_size=4095) -> RegistrationsList:
        ...
        for record in registrations:
            if i >= self.max_registrations_per_list:
                break
            i += 1
    ...
    async def create_registrations_embeds(self, chunk_size=4095) -> RegistrationsEmbeds:
        registrations : wz.RegistrationsRecords = self.registrations
        configured_roles : wz.RolesRecords = self.configured_roles
        if not registrations:
            return None
        ...
        permanent_registration = f"{Emojis.perma_registration.value}: {sum(1 for record in registrations if record.role.id in permanent_ids)}"
        non_permanent_registration = f"{Emojis.normal_registration.value}: {sum(1 for record in registrations if record.role.id in non_permanent_ids)}"
    ...
    async def create_registration_embed(self) -> Embed:
        ...
        try:
            ...
            embed.set_footer(text=f"|", icon_url=self.client_avatar.url if self.client_avatar else None)
            embed.timestamp = utils.utcnow()
            self.logger.debug(f"{self.log_context} Generated registration embed (Default Title: {isDefaultTitle}, Default Description: {isDefaultDescription})")
            return embed
        except Exception as e:
            self.logger.exception(f"{self.log_context} Failed to generate registration embed: {e}")
            return Embed(title=title, description=description, color=self.client_color)
    ...
    async def create_registration_view(self) -> View :
        try:
            ...
            for record in configured_roles:
                ...
                row += 1
                self.logger.debug(f"{self.log_context} Added registration role button for role {record.role.id if record.role else 'unknown'} to view.")

            return view
        except Exception as e:
            self.logger.exception(f"{self.log_context} View Error: {e}")
    ...
    async def registration_register(self, interaction: Interaction, role: Role):
        try:
            ...
            self.logger.debug(f"{self.log_context} {interaction.user} registration action({action}) for role {role.id}.")
            ...
            self.logger.exception(f"{self.log_context} {interaction.user} failed to register for role {role.name}: {e}")
                ...
                for i in range(max(len_embeds, len_records)):
                    ...
                    if i < len_embeds and i < len_records:
                        try:
                            ...
                            self.logger.warning(f"{self.log_context} Registrations List Overview update warning: Failed to update message {list_records[i].message.id}: {e}")
                    elif i < len_embeds:
                        try:
                            ...
                            self.logger.warning(f"{self.log_context} Registrations List Overview update warning: Registration channel {self.registration.channel.id} not found for sending new message.")
                        except Forbidden:
                            self.logger.warning(f"{self.log_context} Registrations List Overview update warning: Missing permissions to access registration channel {self.registration.channel.id} for sending new message.")
                        except HTTPException as e:
                            self.logger.warning(f"{self.log_context} Registrations List Overview update warning: HTTP error while sending new message in registration channel {self.registration.channel.id}: {e}")
                        except ValueError as e:
                            self.logger.warning(f"{self.log_context} Registrations List Overview update warning: Failed to add new message to database: {e}")
                    elif i < len_records:
                        try:
                            await list_records[i].message.delete()
                        except NotFound:
                            self.logger.warning(f"{self.log_context} Registrations List Overview update warning: Message {list_records[i].message.id} not found for deletion.")
                        except Forbidden:
                            self.logger.warning(f"{self.log_context} Registrations List Overview update warning: Missing permissions to delete message {list_records[i].message.id}.")
                        except HTTPException as e:
                            self.logger.warning(f"{self.log_context} Registrations List Overview update warning: HTTP error while deleting message {list_records[i].message.id}: {e}")
                        await self.services.wz.list.remove(guild=self.guild, message=list_records[i].message.id)

                return True
            else:
                self.logger.debug(f"{self.log_context} Cannot update registrations list overview: Invalid setup.")
                return False
        except Exception as e:
            self.logger.exception(f"{self.log_context} Registrations List Overview update failed: {e}")
            return False
    ...
    async def sync(self) -> bool:
        try:
            ...
            self.logger.info(f"{self.log_context} Registration Overview: synced successfully.")
            return True
        except Exception as e:
            self.logger.exception(f"{self.log_context} Failed to sync registration overview: {e}")
            return False
        finally:
        ...
        try:
            ...
            for i in range(3):
                try:
                    ...
                    self.logger.exception(f"{self.log_context} Failed to send registration overview during ensure attempt {i+1}: {e}")
        except Exception as e:
            self.logger.exception(f"{self.log_context} Failed to ensure registration overview: {e}")
            return False
                            ...
                            for member in role.members:
                                try:
                                    if not member.id in {record.member.id for record in registrations}:
                                        ...
                                        self.logger.info(f"{self.log_context} Synced registration for member {member.id} with role {role.id}.")                        
                                except (HTTPException, Forbidden, NotFound, InteractionResponded) as e:
                                    self.logger.warning(f"{self.log_context} Failed to sync registration for member {member.id}: {e}")
                    except Forbidden:
                        self.logger.warning(f"{self.log_context} Missing permissions to manage roles during registration sync.")
                    except NotFound:
                        self.logger.warning(f"{self.log_context} Role not found during registration sync.")
                    except HTTPException as e:
                        self.logger.warning(f"{self.log_context} HTTP error during registration sync: {e}")
            self.logger.info(f"{self.log_context} Registration overview repaired successfully.")
            return True
        except Exception as e:
            self.logger.exception(f"{self.log_context} Failed to repair registration overview: {e}")
            return False
        finally:
        ...
        try:
            ...
            if self.is_valid_setup:
                ...
                try:
                    ...
                    self.logger.warning(f"{self.log_context} Failed to delete existing registration overview message during send: {e}")
                try:
                    await self.services.wz.registration.setup_registration(
                        guild=self.guild,
                        message=None
                    )
                    self.logger.debug(f"{self.log_context} Cleared registration overview message reference in database after failed deletion.")
                except Exception as e:
                    self.logger.exception(f"{self.log_context} Failed to clear registration overview message reference in database after failed deletion: {e}")
                ...
                try:
                    ...
                    self.logger.warning(f"{self.log_context} Failed to send registration overview message during send: {e}")
                    return False
            return False
        finally:
            await self.work_stop()
    ...
    async def update(self) -> bool:
        try:
            ...
            try:
                ...
                self.logger.warning(f"{self.log_context} Failed to update registration overview message: {e}")
                return False        
        except Exception as e:
            self.logger.exception(f"{self.log_context} Failed to update registration overview: {e}")
            return False
        finally:
            await self.work_stop()
                    ...
                    async for msg in registration.channel.history(limit=999):
                        if msg.author.id != self.client.user.id:
                            try:
                                ...
                                self.logger.warning(f"{self.log_context} Missing permissions to delete messages in registration channel {registration.channel.id}.")
                                return False
                            except NotFound:
                                self.logger.warning(f"{self.log_context} Registration channel {registration.channel.id} not found for deleting messages.")
                                return False
                            except HTTPException as e:
                                self.logger.warning(f"{self.log_context} HTTP error while deleting messages in registration channel {registration.channel.id}: {e}")
                                return False
                except Exception as e:
                    self.logger.exception(f"{self.log_context} Failed to clean registration channel: {e}")
                    return False
                return True
            return False
        except Exception as e:
    ...
    async def delete(self) -> bool:
        try:
            ...
            self.logger.exception(f"{self.log_context} Failed to delete registration messages: {e}")
            return False
        finally:
