...
async def message(interaction: Interaction, title: Optional[str] = None, message: Optional[str] = None, message_upload: Optional[Attachment] = None): 
    MESSAGES = {
        "SUCCESS": f"{Emojis.SUCCESS} Nachricht für die WZ-Registrierung wurde erfolgreich festgelegt.",
        "NO_INPUT": f"{Emojis.ERROR} Übergebe zumindest eine der Optionen: title, message oder message_upload.",
        "TWO_MESSAGES": f"{Emojis.ERROR} Bitte übergebe nur eine der Optionen: message oder message_upload. title kann optional mit einer der beiden übergeben werden.",
        "ERROR": f"{Emojis.ERROR} Fehler beim Festlegen der Nachricht für die WZ-Registrierung.",
        "ERROR_FILE_TYPE": f"{Emojis.ERROR} Ungültiger Dateityp. Bitte lade eine Textdatei (.txt) hoch."
    }
