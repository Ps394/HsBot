...
async def add(interaction: Interaction, role: Role, permanent: Optional[bool] = False):
    MAX_ROLES = 4
    MESSAGES = {
        "ERROR": f"{Emojis.ERROR} Fehler beim Hinzufügen der Rolle zur WZ-Registrierung.",
        "MAX_ROLES": f"{Emojis.WARNING} Es können maximal {MAX_ROLES} Rollen für die WZ-Registrierung festgelegt werden.",
        "SUCCESS": f"{Emojis.SUCCESS} Rolle {{role_name}} wurde zur WZ-Registrierung hinzugefügt."
    }
